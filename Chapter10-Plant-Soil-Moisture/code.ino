...
void loop() {
  ...
  // Update RGB LED color based on moisture level
   if (currentMillis - lastRgbUpdate >= RGB_UPDATE_INTERVAL) {
    ...
    if (constrainedSensorValue <= point1) {
  ...
  // Blink red LED if moisture is low
  ...
  // Update LCD only if moisture changes
